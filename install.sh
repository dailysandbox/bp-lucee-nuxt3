...
# Prompt for the Lucee password
read -p "Enter the Lucee admin password [123456]: " lucee_admin_password_input
lucee_admin_password=${lucee_admin_password_input:-"123456"}
...
for file_pair in "${files[@]}"; do
    ...
    if [[ -f "$template_file" ]]; then
        ...
        sed "s/{{DOMAIN_NAME}}/${domain_name}/g; s/{{PROJECT_NAME}}/${project_name}/g; s/{{LUCEE_ADMIN_PASSWORD}}/${lucee_admin_password}/g" "$template_file" > "$output_file"
    else
...
DOMAIN_URL="https://api.${domain_name}"
